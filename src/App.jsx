...
const cvUrl = `${import.meta.env.BASE_URL}MD_Meraz_Ahasan_Shah_Resume.pdf`;
...
const educationItems = [
  {
    ...
    result: "Expected Graduation: 2026",
  ...
  },
  {
    ...
    place: "Cantonment Public School and College",
    result: "GPA: 4.67 / 5.00",
  },
  {
    period: "2017",
    title: "Secondary School Certificate (SSC)",
    place: "Lions School and College",
    ...
    <header className="saas-nav-wrap">
      <nav className="saas-nav" aria-label="Primary navigation">
        ...
        <div className="saas-nav-actions">
          ...
          >
            {isDay ? <Moon className="h-5 w-5" aria-hidden="true" /> : <Sun className="h-5 w-5" aria-hidden="true" />}
          </button>
        </div>
        <button
          type="button"
  ...
  return (
    <section id="home" className="saas-section saas-hero">
      <motion.div initial="hidden" animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: 0.08 } } }} className="saas-hero-copy">
        <motion.h1 variants={{ hidden: { opacity: 0, y: 14 }, show: { opacity: 1, y: 0 } }}>Software Engineer building clean, scalable web products.</motion.h1>
        ...
        <motion.div variants={{ hidden: { opacity: 0, y: 14 }, show: { opacity: 1, y: 0 } }} className="saas-hero-actions">
          ...
          <a href={cvUrl} className="saas-btn saas-btn-secondary" download>Download Resume <Download className="h-4 w-4" /></a>
  ...
  return (
    <div id="about" className="saas-about">
      <motion.div {...fadeUp} className="saas-about-copy">
        ...
        <p>
          I am MD Meraz Ahasan Shah, a Software Engineer and CSE student at AIUB. I build practical web applications with clear UI, protected backend flows, database-backed features, and recruiter-ready project presentation.
        </p>
        <p>
...
function BackgroundSection({ id, title, description, items, icon: Icon }) {
  return (
    <section id={id} className="saas-section" aria-label={title}>
      <SectionHeader eyebrow={title} title={description} />
      <div className="saas-background-grid">
        {items.map((item) => (
          <motion.article key={item.title} {...fadeUp} className="saas-edu-exp-panel saas-background-card">
            <div className="saas-background-meta">
              <Icon className="h-5 w-5" aria-hidden="true" />
              <time>{item.period}</time>
            </div>
            <h3>{item.title}</h3>
            <strong>{item.place}</strong>
            {item.result && <span className="saas-education-result">{item.result}</span>}
  ...
  return (
    <footer className="saas-section saas-footer">
      <div>
        ...
        <p>Software Engineer building clean, scalable web applications and recruiter-ready product projects.</p>
        <a href={cvUrl} className="saas-resume-link" download>Download Resume <Download className="h-3.5 w-3.5" /></a>
      </div>
      <div>
        <h3>Navigation</h3>
